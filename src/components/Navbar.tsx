...
import { Droplets, Truck, Menu, X, ArrowRight, ChevronRight, Phone, MessageCircle, Smartphone } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ 
  ...
  const navLinks = [
    ...
    { name: language === 'ar' ? 'كيف تعمل المنصة' : 'How It Works', href: '/how-it-works' },
    { name: language === 'ar' ? 'تطبيق العميل' : 'Customer App', href: '/how-it-works/customer' },
    { name: language === 'ar' ? 'تطبيق السائق' : 'Driver App', href: '/how-it-works/driver' },
    { name: language === 'ar' ? 'الصهاريج' : 'Tankers', href: '/how-it-works/tanker' },
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#060c18]/95 border-b border-cyan-500/15 transition-all">
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="sm:hidden border-t border-slate-800 bg-[#071122]/98 backdrop-blur-2xl px-4 pt-3 pb-6 space-y-2 animate-in slide-in-from-top-4 duration-200">
          ...
          <div className="grid grid-cols-2 gap-2 pb-2">
            {navLinks.map((link) => (
              <Link
                ...
                className="px-3 py-2 text-xs text-slate-300 hover:text-cyan-300 hover:bg-slate-800/60 rounded-md font-medium flex items-center justify-between"
              >
                <span>{link.name}</span>
